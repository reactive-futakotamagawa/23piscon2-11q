...
func getTrend(c echo.Context) error {
	...
	for _, character := range characterList {
		...
		err = db.Select(&conditions,
			"SELECT `isu`.`id` AS isuID,`isu_condition`.* FROM `isu` JOIN `isu_condition` ON `isu`.`jia_isu_uuid` = `isu_condition`.`jia_isu_uuid`	WHERE `isu`.`character` = ? AND `isu_condition`.`id` IN (select max(id) from `isu_condition` group by jia_isu_uuid) ORDER BY `isu_condition`.`timestamp` DESC",
			character.Character)
		if err != nil {
			c.Logger().Errorf("db error: %v", err)
			return c.NoContent(http.StatusInternalServerError)
